...
async function startServer() {
  try {
    // Connect to database
    logger.info('Starting Prescription Generation Service...');
    ...
    const server = app.listen(PORT, () => {
      logger.info('Prescription Generation Service started', {
        port: PORT,
...
function validateEnvironment() {
  ...
  const booleanVars = ['ENABLE_DRUG_INTERACTION_CHECK', 'ENABLE_DOSAGE_VALIDATION'];
  booleanVars.forEach(varName => {
    const value = process.env[varName];
    if (value && !['true', 'false'].includes(value.toLowerCase())) {
      logger.warn(`${varName} should be 'true' or 'false', got: ${value}`);
    }
  });
